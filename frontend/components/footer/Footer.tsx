      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-12 px-6 py-14 lg:px-10">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_repeat(4,1fr)]">
          <div className="flex flex-col gap-4">
            ...
            <a
              ...
              className="inline-flex w-fit items-center gap-2 rounded-pill border border-line bg-white/[0.04] px-4 py-2 text-sm text-frost-dim transition-colors hover:border-white/20 hover:text-frost"
            >
          ...
          {siteConfig.footerNav.map((group) => (
            <nav key={group.title} aria-label={group.title}>
              <h2 className="text-xs font-semibold uppercase tracking-wider text-frost">
                {group.title}
              </h2>
                  ...
                  <li key={link.href}>
                    {"external" in link && link.external ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer noopener"
                        className="text-sm text-mute transition-colors hover:text-frost"
                      >
                        {link.label}
                      </a>
                    ) : (
        ...
        <div className="flex flex-col gap-3 border-t border-line/60 pt-6">
          ...
          <p className="text-xs leading-relaxed text-mute">
            ...
            Gatekeeper approval.{" "}
            <Link href="/install" className="text-frost-dim underline underline-offset-4 hover:text-frost">
